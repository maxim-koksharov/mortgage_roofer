...
pub fn fetch_euribor(tenor: EuriborTenor) -> Result<f64, String> {
    // ECB SDMX API endpoint for Euribor.
    // Series key format: M.EXT_EURIBOR_6M (for 6-month Euribor)
    ...
    let value = json
        ...
        .and_then(|v| v.get(0))
        .and_then(|v| v.as_f64())
        .ok_or("Could not extract Euribor value from response")?;
...
impl EuriborCache {
    ...
    pub fn get_or_fetch(&mut self, tenor: EuriborTenor) -> Result<f64, String> {
